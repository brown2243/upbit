...
async function init() {
  ...
  setTimeout(() => {
    serverStart(coinInfo, wallet);
  }, cheapCoins.length * 300);
}

async function serverStart(coinInfo, wallet) {
  const code_list = Object.keys(coinInfo).map((v) => `"${v}"`);
  console.log("체크하는 코인 갯수: " + code_list.length);
  console.log(checkList);
  console.log(wallet);
  console.log(coinInfo);
  tradeServerConnect(code_list.join(","), coinInfo, wallet);
}

function tradeServerConnect(codes, coinInfo, wallet) {
  ...
  ws.on("close", () => {
    ...
    setTimeout(function () {
      ...
      tradeServerConnect(codes, coinInfo, wallet);
    }, 1000);
  });
